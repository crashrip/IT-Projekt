...
namespace OLAP_WindowsForms.App
{
    public partial class UserInput : Form
    {
        ...
        public void insert()
        {
            ...
            if (dim_doctor) { DBContext.Service().insertDimQual(CDW_DOCTOR, CDW_DOCTOR_GL, TDW_DOCTOR,LDW_DOCTOR, id, 1,"Doctor", doctor_DL, doctor_DN,doctor_SC, doctor_GL); }
            if (dim_insurance) { DBContext.Service().insertDimQual(CDW_INSURANT, CDW_INSURANT_GL, TDW_INSURANT,LDW_INSURANT, id, 2,"Insurant", insurant_DL, insurant_DN,insurant_SC, insurant_GL); }
            if (dim_drug) { DBContext.Service().insertDimQual(CDW_DRUG, CDW_DRUG_GL, TDW_DRUG,LDW_DRUG, id, 3,"Drug", drug_DL, drug_DN,drug_SC, drug_GL); }
            if (dim_medservice) { DBContext.Service().insertDimQual(CDW_MEDSERVICE, CDW_MEDSERVICE_GL, TDW_MEDSERVICE,LDW_MEDSERVICE, id, 4,"MedService", meds_DL, meds_DN,meds_SC, meds_GL); }
            if (dim_hospital) { DBContext.Service().insertDimQual(CDW_HOSPITAL, CDW_HOSPITAL_GL, TDW_HOSPITAL, LDW_HOSPITAL, id, 5, "Hospital", hospital_DL, hospital_DN, hospital_SC,hospital_GL); }
            if (dim_time) { DBContext.Service().insertDimQual(CDW_TIME, CDW_TIME_GL, TDW_TIME,LDW_TIME, id, 6,"Time", time_DL, time_DN,time_SC, time_GL); }
